...
# Method to insert new line after every 8th character
...
# Per MLG's comments from Week 3, I searched for a more explicit way
# to format large numbers with sprintf, but didn't find one.
# Am I missing something?
def comma_adder(num)
  sprintf(num.to_s).gsub(/(\d)(?=(\d\d\d)+(?!\d))/, "\\1,")
end

# Output each username followed by description ("NA" if nil)
puts "USERNAMES & DESCRIPTIONS\n#{sep2}"
twitter_data.each_with_index do |key, index|
  name = twitter_data[index].keys
  description = twitter_data[index][name[0]]["description"]
  print "USR: #{name.join}\nDES: "
  if description == nil
    print "N/A"
  else
    text_formater(description)
  end
  print "\n-----------------\n"
end

# Output each username & total number of followers
puts "#{sep2}\nNUMBER OF FOLLOWERS:\n#{sep2}"
twitter_data.each_with_index do |key, index|
  name = twitter_data[index].keys
  number_followers = twitter_data[index][name[0]]["number of followers"]
  puts "#{name.join} ==> #{number_followers} Followers"
end

# Output each username & length of their latest tweet
puts "#{sep2}\nLENGTH OF LATEST TWEET:\n#{sep2}"
twitter_data.each_with_index do |key, index|
  name = twitter_data[index].keys
  length_last_tweet = twitter_data[index][name[0]]["last twenty tweets"][0].length
  puts "#{name.join} ==> #{length_last_tweet} Characters"
end

# Output each username & sum of character count in user's last twenty tweets
puts "#{sep2}\nLENGTH OF LAST 20 TWEETS:\n#{sep2}"
twitter_data.each_with_index do |key, index|
  total_character_count = 0
  name = twitter_data[index].keys
  twitter_data[index][name[0]]["last twenty tweets"].each_with_index do |key_02, index_02|
    tweet_character_count = twitter_data[index][name[0]]["last twenty tweets"][index_02].length
    total_character_count += tweet_character_count
  end
  puts "#{name.join} ==> #{comma_adder(total_character_count)} Characters"
end

# Find user with the most followers
most_followers_name = ""
most_followers_number = 0
twitter_data.each_with_index do |key, index|
  name = twitter_data[index].keys
  number_followers = twitter_data[index][name[0]]["number of followers"]
  if number_followers > most_followers_number
    most_followers_name = name
    most_followers_number = number_followers
  end
end
puts "#{sep2}\nUSER WITH THE MOST FOLLOWERS:"
puts "#{most_followers_name.join} ==> #{comma_adder(most_followers_number)} Followers"

# Find user with the most friends
most_friends_name = ""
most_friends_number = 0
twitter_data.each_with_index do |key, index|
  name = twitter_data[index].keys
  number_friends = twitter_data[index][name[0]]["number of friends"]
  if number_friends > most_friends_number
    most_friends_name = name
    most_friends_number = number_friends
  end
end
puts "#{sep2}\nUSER WITH THE MOST FRIENDS:"
puts "#{most_friends_name.join} ==> #{comma_adder(most_friends_number)} Friends"

# Find user with the greatest number of tweets
most_tweets_name = ""
most_tweets_number = 0
twitter_data.each_with_index do |key, index|
  name = twitter_data[index].keys
  number_tweets = twitter_data[index][name[0]]["number of tweets"]
  if number_tweets > most_tweets_number
    most_tweets_name = name
    most_tweets_number = number_tweets
  end
end
puts "#{sep2}\nUSER WITH THE MOST TWEETS:"
puts "#{most_tweets_name.join} ==> #{comma_adder(most_tweets_number)} Tweets\n#{sep2}"

# List all users with a description in twitter_data
...
# List all users with a location in twitter_data?
puts "USERS WITH LOCATION LISTED:\n#{sep2}"
twitter_data.each_with_index do |key, index|
  name = twitter_data[index].keys
  location = twitter_data[index][name[0]]["location"]
  if location != nil
    puts name
  end
end
